	...
	var $w = $(window),
		...
		_nmObj = {
			...
			resize: function(recalc, w) {
				...
				this._callAnim('resize', $.proxy(function() {
					...
					this._reposition();
					try{
						$('.nyroModalCont').width($('.nyroModalCont').width()+modalResizeWidth);
							
					}catch(e){alert(e);}
					
					if( $(window).height() < $('.nyroModalLink').height()+10){
						
					}else{
						if(w > $('.nyroModalLink').width()){
		...
		_internal = {
			...
			nmManual: function(url, opts) {
				if(url.indexOf("/common/pop_systemAgree") > -1){
