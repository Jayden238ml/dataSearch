...
<script type="text/javascript">
	...
	function fnDetail(seq, b_seq01, b_seq02, b_seq03){
		$('#MST_BOARD_SEQ').val(seq);
		$('#BOARD_SEQ01').val(b_seq01);
		$('#BOARD_SEQ02').val(b_seq02);
		$('#BOARD_SEQ03').val(b_seq03);
				...
				<div id="cont">
					<section class="sect_area sect_first_area">
						...
						<form name="frm" id="frm" method="post" action="#">
							...
							<input type="hidden" name="RTN_URL" id="RTN_URL" value="1" />
							...
							<input type="hidden" id="MST_BOARD_SEQ"  name="MST_BOARD_SEQ" value=""/>
							<input type="hidden" id="BOARD_SEQ01"  name="BOARD_SEQ01" value=""/>
							<input type="hidden" id="BOARD_SEQ02"  name="BOARD_SEQ02" value=""/>
							<input type="hidden" id="BOARD_SEQ03"  name="BOARD_SEQ03" value=""/>
							<fieldset>
								<div class="searchCmn">
										...
										<ul>
											<li class="twoFL">
												...
												<select name="SCH_TYPE" id="SCH_TYPE" class="select" title="검색분류">
													...
													<option value="B" <c:if test="${INIT_DATA.SCH_TYPE eq 'B' }">selected</c:if>>단지명</option>
								...
								<tbody>
									<c:if test="${not empty INIT_DATA.resultList}">
										<c:forEach items="${INIT_DATA.resultList}" var="item" varStatus="status">
											<tr onclick="fnDetail('${item.MST_BOARD_SEQ}', '${item.BOARD_SEQ01}','${item.BOARD_SEQ02}', '${item.BOARD_SEQ03}');" style="cursor:pointer;">
												<td>${item.RNUMUM}</td>
