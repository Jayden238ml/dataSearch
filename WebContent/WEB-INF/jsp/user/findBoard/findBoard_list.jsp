...
function fnBoardWrite(){
	if("${INIT_DATA.SESSION_USR_ID}" == ""){
		...
		$('#ListFrm').attr("action", "/user/Board/findBoardWrite.do").submit();	
	}
...
function fnDetail(seq){
	$('#FIND_SEQ').val(seq);
	$('#ListFrm').attr("action", "/user/Board/findBoardView.do").submit();
}
</script>
</head>
<body>
	<div id="wrap">
		...
		<div id="content">
			<div id="rightCnt">
				...
				<input type="hidden" name="FIND_SEQ"	id="FIND_SEQ"	value=""/>
								...
								<c:forEach items="${INIT_DATA.boardList }" var="item" varStatus="rowStatus">
									<tr>
										<td style="text-align:left;">
											<a href="javascript:fnDetail('<c:out value="${item.FIND_SEQ}" />');"><c:out value="${item.TITLE }"/>&nbsp;&nbsp;</a>
