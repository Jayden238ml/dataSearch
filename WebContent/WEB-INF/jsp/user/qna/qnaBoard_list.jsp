...
function fnSearch(){
	$('#ListFrm').attr("action", "/user/Board/qnaBoard.do").submit();	
}
...
function fnBoardWrite(){
	if("${INIT_DATA.SESSION_USR_ID}" == ""){
		...
		$('#ListFrm').attr("action", "/user/Board/storyWrite.do").submit();	
	}
...
function fnDetail(seq){
	$('#QNA_SEQ').val(seq);
	$('#ListFrm').attr("action", "/user/Board/storyView.do").submit();
}
</script>
</head>
<body>
	<div id="wrap">
		...
		<div id="content">
			<div id="rightCnt">
				...
				<input type="hidden" name="QNA_SEQ"	id="QNA_SEQ"	value=""/>
								...
								<c:forEach items="${INIT_DATA.boardList }" var="item" varStatus="rowStatus">
									<tr>
										<td style="text-align:left;">
											<a href="javascript:fnDetail('<c:out value="${item.QNA_SEQ}" />');"><c:out value="${item.TITLE }"/>&nbsp;&nbsp;</a>
