...
function fnAddRip(){
	if("${INIT_DATA.SESSION_USR_ID}" == ""){
		...
		$.ajax({
			 type		: "POST"
			,url		: "/user/Board/qnaRippleWrite.do"
			,dataType	: "json"
			...
			,success : function(transport) {
				if(transport.ERROR_CD == '999'){
					...
					$('#boardFrm').attr("action", "/user/Board/storyView.do").submit();	
				}
			}
		});
	}
...
function fnmodify(){
	$('#boardFrm').attr("action", "/user/Board/storyWrite.do").submit();	
}
...
function fnDelete(){
	if(confirm("삭제하시겠습니까?")){
		$.ajax({
			 type		: "POST"
			,url		: "/user/Board/qnaBoardDelete.do"
			,dataType	: "json"
			,data: $("#boardFrm").serialize()
			,success : function(transport) {
				if(transport.ERROR_CD == '999'){
...
function fnRiDelete(r_seq, rr_seq,  sort){
	...
	if(confirm("삭제하시겠습니까?")){
		$.ajax({
			 type		: "POST"
			,url		: "/user/Board/qnaBoardRipDelete.do"
			,dataType	: "json"
			,data: $("#boardFrm").serialize()
			,success : function(transport) {
				if(transport.ERROR_CD == '999'){
	...
	<div id="wrap">
		...
		<div id="content">
			...
			<form name="boardFrm" id="boardFrm" method="post" action="#" class="form-inline">
				<input type="hidden" name="QNA_SEQ" id="QNA_SEQ" value="<c:out value='${INIT_DATA.detail.QNA_SEQ}' />" />
