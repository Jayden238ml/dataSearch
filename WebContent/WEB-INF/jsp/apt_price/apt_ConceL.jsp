...
<script type="text/javascript">
	...
	function fnDetail(apt_nm, code, jibun, area){
		$('#APARTMENT_NAME').val(apt_nm);
		$('#REGIONAL_CODE').val(code);
		$('#JIBUN').val(jibun);
		...
		<ul id="content">
			...
			<li id="right">
				...
				<div class="titleNaviNew">
					<h3 class="subTitle">아파트분양권 거래이력</h3>
				</div>
				<!-- 타이틀/네비 끝 -->
				...
				<div id="cont">
					<section class="sect_area sect_first_area">
						...
						<form name="frm" id="frm" method="post" action="#">
							...
							<input type="hidden" name="RTN_URL" id="RTN_URL" value="/user/apt_ConceL.do" />
							<input type="hidden" name="APARTMENT_NAME" id="APARTMENT_NAME" value="" />
							<input type="hidden" name="REGIONAL_CODE" id="REGIONAL_CODE" value="" />
							<input type="hidden" name="JIBUN" id="JIBUN" value="" />
...
<%-- 												<td><a href="javascript:fnDetail('${item.APARTMENT_NAME}', '${item.REGIONAL_CODE}', '${item.JIBUN}', '${item.AREA_EXCLUSIVE_USE}');">${item.SIGUNGU}&nbsp;${item.JIBUN}</a></td> --%>
												...
												<td><a href="javascript:fnDetail('${item.APARTMENT_NAME}', '${item.REGIONAL_CODE}', '${item.JIBUN}', '${item.AREA_EXCLUSIVE_USE}');">${item.DEAL_YEAR}년&nbsp;${item.DEAL_MONTH}월&nbsp;${item.DEAL_DAY}일</a></td>
											</tr>
										</c:forEach>
									</c:if>
