...
<script type="text/javascript">
	...
	function fnSearch(){
		$('#frm').attr("action", "/user/apt_CompareL.do");
		$('#frm').submit();
	}
	...
	function fnSearchMyhome(gubun){
		...
		$.ajax({
			type: "POST",
			url : "/api/serachComPareAptDeal_List.do",
			data:  $("#frm").serialize(),
			dataType: "json",
	...
	function fnsetMyGrid(dataSet, gubun){
		...
		if (dataSet == null || dataSet == "") {
			...
			$.each(dataSet,function(key,json) {
				...
				if(gubun == "M"){
					resultHtml += '					<td><a href="javascript:fnMySet(\'' + json.APARTMENT_NAME + '\',\'' + json.ROAD_NAME_BONBUN + '\',\'' + json.AREA_EXCLUSIVE_USE +'\');" class="mBtn3">선택</a> </td>';
				}else{
					resultHtml += '					<td><a href="javascript:fnYouSet(\'' + json.APARTMENT_NAME + '\',\'' + json.ROAD_NAME_BONBUN + '\',\'' + json.AREA_EXCLUSIVE_USE +'\');" class="mBtn3">선택</a> </td>';
				}
				resultHtml += '				</tr>';
				resultHtml += '			</thead>';
				resultHtml += '		</table>';
	...
	function fnMySet(a_nm, bonbun, area){
		$('#MY_APARTMENT_NAME').val(a_nm);
		$('#MY_ROAD_NAME_BONBUN').val(bonbun);
		$('#MY_AREA_EXCLUSIVE_USE').val(area);
		$('#mydataBody').html("");
		$('#mytr').show();
		$('#mytd').html(a_nm + '(' + area + ')');
		
		if($('#MY_ROAD_NAME_BONBUN').val() != "" && $('#YOU_ROAD_NAME_BONBUN').val() != ""){
			$('#compareBtn').show();
		}else{
			$('#compareBtn').hide();
		}
	...
	function fnYouSet(a_nm, bonbun, area){
		$('#YOU_APARTMENT_NAME').val(a_nm);
		$('#YOU_ROAD_NAME_BONBUN').val(bonbun);
	...
	function fnReset(){
		...
		$('#MY_ROAD_NAME_BONBUN').val("");
		$('#MY_AREA_EXCLUSIVE_USE').val("");
		$('#YOU_APARTMENT_NAME').val("");
		$('#YOU_ROAD_NAME_BONBUN').val("");
				...
				<div id="cont">
					<section class="sect_area sect_first_area">
						...
						<form name="frm" id="frm" method="post" action="#">
							...
							<input type="hidden" name="MY_ROAD_NAME_BONBUN" id="MY_ROAD_NAME_BONBUN" value="${INIT_DATA.MY_ROAD_NAME_BONBUN}" />
