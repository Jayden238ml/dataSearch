...
<script type="text/javascript">
	...
	function fnDetail(seq, dong, hosu){
		...
		$('#DONG').val(dong);
		$('#HOSU').val(hosu);
		$('#frm').attr("action", "/apt/apt_warrantDetail.do");
		$('#frm').submit();
	}
	
	function fnExcelDown(){
		$('#frm').attr("target", "_self");
		$('#frm').attr("action", "/apt/ExcelDown.do");
		$('#frm').submit();
	}
	
	function fnTelExcelDown(){
		$('#frm').attr("target", "_self");
		$('#frm').attr("action", "/apt/LmsExcelDown.do");
		$('#frm').submit();
	}
	
	function fnExcelPop(){
		gfnOpenLayerPopup('/common/index.do?jpath=/ourapt/pop_WarrantExcelUpload');
	}
		...
		<ul id="content">
			...
			<li id="right">
				...
				<div class="titleNaviNew">
					<h3 class="subTitle">[${INIT_DATA.detail.APT_NM}]&nbsp;위임장 관리</h3>
				</div>
				<!-- 타이틀/네비 끝 -->
						...
						<dl class="titleEaB">
							...
							<dd>
								<c:if test="${INIT_DATA.ISMOBILE eq 'P'}">
									<a href="javascript:fnExcelPop();" class="mBtn3">위임장 내역 업로드</a>
									<a href="javascript:fnExcelDown();" class="mBtn4">내역다운로드</a>
									<a href="javascript:fnTelExcelDown();" class="mBtn5">연락처 다운로드</a>
								</c:if>
							</dd> 
						</dl>
						<!-- 리스트 -->
								...
								<tbody>
									<c:if test="${not empty INIT_DATA.resultList}">
										<c:forEach items="${INIT_DATA.resultList}" var="item" varStatus="status">
											<tr>
												<td><a href="javascript:fnDetail('${item.WARRANT_SEQ}', '${item.DONG}', '${item.HOSU}');">${item.DONG}동</a></td>
												<td><a href="javascript:fnDetail('${item.WARRANT_SEQ}', '${item.DONG}', '${item.HOSU}');">${item.HOSU}호</a></td>
												<td><a href="javascript:fnDetail('${item.WARRANT_SEQ}', '${item.DONG}', '${item.HOSU}');">${item.USER_NM}</a></td>
												<td>${item.HP}</td>
												<td><a href="javascript:fnDetail('${item.WARRANT_SEQ}', '${item.DONG}', '${item.HOSU}');">${item.WARRANT_YN_NM}</a></td>
												<td><a href="javascript:fnDetail('${item.WARRANT_SEQ}', '${item.DONG}', '${item.HOSU}');">${item.AMT_YN_NM}</a></td>
											</tr>
										</c:forEach>
									</c:if>
