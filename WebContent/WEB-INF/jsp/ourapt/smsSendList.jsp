...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#popFrm').find("input").each(function(e){
	...
	function fnSearch(){
		$('#popFrm').attr("action", "/apt/smsSend.do");
		$('#popFrm').submit();
	}
	...
	function fnDetail(seq, dong, hosu){
		...
		$('#DONG').val(dong);
		$('#HOSU').val(hosu);
		$('#popFrm').attr("action", "/apt/apt_warrantDetail.do");
		$('#popFrm').submit();
	}
	
	function fnExcelDown(){
		$('#popFrm').attr("target", "_self");
		$('#popFrm').attr("action", "/apt/ExcelDown.do");
		$('#popFrm').submit();
	}
	
	function fnTelExcelDown(){
		$('#popFrm').attr("target", "_self");
		$('#popFrm').attr("action", "/apt/LmsExcelDown.do");
		$('#popFrm').submit();
	}
	
	function fnSmsPop(){
		if($('#SEND_TEL').val() == ""){
			alert("발신번호 등록요청 후 발송가능합니다.");
			return;
		}
		var type = "P"
		if("${INIT_DATA.ISMOBILE}" == "M"){
			type = "M"
		}
		$('#TYPE').val(type);
		gfnOpenMoveLayerPopup('/apt/smsSendPop.do');
	}
		...
		<ul id="content">
			...
			<li id="right">
				...
				<div class="titleNaviNew">
					<h3 class="subTitle">[${INIT_DATA.detail.APT_NM}]&nbsp;문자발송</h3>
				</div>
				<!-- 타이틀/네비 끝 -->
				...
				<div id="cont">
					<section class="sect_area sect_first_area">
						...
						<form name="popFrm" id="popFrm" method="post" action="#">
							...
							<input type="hidden" name="SEND_TEL" id="SEND_TEL" value="${INIT_DATA.detail.SEND_TEL}" />
						...
						<dl class="titleEaB">
							<dt>
								금액이 부족할 경우<span style="color:#2b70c9;">케이뱅크 700-100-011154 (주식회사 올마이티컴퍼니)</span> 로 충전 후 발송이 가능 합니다.
							</dt>
						</dl>
						<dl class="titleEaB">
							<dt>
								현재 잔액&nbsp; :&nbsp;  <strong><fmt:formatNumber value="${INIT_DATA.NOW_AMT}" pattern="#,###"/></strong> 원&nbsp;
							</dt>
							<dd>
								<a href="javascript:fnSmsPop();" class="mBtn3">문자전송</a>
							</dd> 
						</dl>
						<!-- 리스트 -->
